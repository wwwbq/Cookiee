...
from .configuration_llava_vlm import LlavaVlmConfig
...
_CONFIG_FOR_DOC = LlavaVlmConfig


@dataclass
class LlavaVlmOutputWithPast(ModelOutput):
    """
    Base class for Vlm causal language model (or autoregressive) outputs.
...
class LlavaVlmForConditionalGeneration(LlavaVlmPreTrainedModel, GenerationMixin):
    def __init__(self, config: LlavaVlmConfig):
        ...
        if config.stage == "pretrain":
            ...
            if "clip" in config.vision_model.lower():
                self.vision_tower = CLIPVisionModel.from_pretrained(config.vision_model)
            else:
                self.vision_tower = AutoModel.from_pretrained(config.vision_model)
    ...
    @add_start_docstrings_to_model_forward(Llava_VLM_INPUTS_DOCSTRING)
    @replace_return_docstrings(output_type=LlavaVlmOutputWithPast, config_class=_CONFIG_FOR_DOC)
    def forward(
        self,
        ...
        **lm_kwargs,
    ) -> Union[Tuple, LlavaVlmOutputWithPast]:
        r"""
...
AutoModelForVision2Seq.register(LlavaVlmConfig, LlavaVlmForConditionalGeneration)
